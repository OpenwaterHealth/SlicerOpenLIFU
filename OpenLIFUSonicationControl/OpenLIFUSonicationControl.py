...
class OpenLIFUSonicationControlLogic(ScriptedLoadableModuleLogic):
    ...
    def connect_simulated_interface(self, transducer=None) -> None:
        ...
        When ``transducer`` is given (an :class:`openlifu.xdc.TransducerArray`
        or :class:`openlifu.xdc.Transducer`-like object exposing a ``modules``
        sequence), the simulator's TX module count and per-module user_configs
        are configured to mimic that transducer.
        """
