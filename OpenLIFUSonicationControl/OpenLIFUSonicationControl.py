"""Thin Slicer-module shell for the OpenLIFU Sonication Control page.

The actual widget / logic / test / helper / parameter-node classes live in
``OpenLIFUApp.pages.sonication_control_page`` (part of the OpenLIFU host
module). This file exists only so that Slicer still registers
``OpenLIFUSonicationControl`` as a loadable module, allowing
...
already depends on OpenLIFUSonicationControl; a reverse dep cycles and hangs
...
# Re-export the extracted classes so external ``from OpenLIFUSonicationControl
# import OpenLIFUSonicationControlTest`` (used by OpenLIFUHome) keeps working
# and Slicer can discover the Widget/Logic/Test classes via attribute lookup.
from OpenLIFUApp.pages.sonication_control_page import (  # noqa: F401
    DeviceCompatibility,
    DeviceCompatibilitySeverity,
    OnRunCompletedDialog,
...
class OpenLIFUSonicationControl(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.title = _("OpenLIFU Sonication Control")
