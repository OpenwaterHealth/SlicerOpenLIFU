...
* No tables (target list, VF list, pre-solution list, finalized-plan
  ...
  duplicate that here.
* **No in-page navigation buttons.** Overview is the first page on
  the Planning workflow timeline (SlicerOpenLIFU#642); users advance
  to Target Selection via the timeline strip's second circle in the
  footer or the Next button. Editing surfaces live on their own
  workflow pages -- Overview doesn't need a bespoke jump-off button.
* No Finalize Plan button. Finalize is a workflow-completion action;
  it will land on the workflow toolbar in a follow-up commit for
  SlicerOpenLIFU#634.
