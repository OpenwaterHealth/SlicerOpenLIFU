...
def openlifu_point_to_fiducial(point : "openlifu.geo.Point") -> vtkMRMLMarkupsFiducialNode:
    """Create a fiducial node out of an openlifu.geo.Point, removing any existing nodes that would have the same name.
...
def fiducial_to_openlifu_point_id(fiducial_node:vtkMRMLMarkupsFiducialNode) -> str:
    """Get the openlifu point ID that we would use if we were to convert the given fiducial node to an openlifu.geo.Point"""
...
def fiducial_to_openlifu_point_in_transducer_coords(fiducial_node:vtkMRMLMarkupsFiducialNode, transducer:"SlicerOpenLIFUTransducer", name:Optional[str] = None) -> "openlifu.geo.Point":
    """Given a fiducial node with at least one point, return an openlifu.geo.Point in the local coordinates of the given transducer.
    If name is provided then it will be used as the name of the openlifu.geo.Point. Otherwise we use the label on the control point.
    """
    if fiducial_node.GetNumberOfControlPoints() < 1:
...
def fiducial_to_openlifu_point(fiducial_node:vtkMRMLMarkupsFiducialNode) -> "openlifu.geo.Point":
    """Given a fiducial node with at least one point, return an openlifu.geo.Point in RAS coordinates.
