...
This document defines the maintenance and support policy for `SlicerOpenLIFU`.

## Release lines and patch releases

A **release line** is a `MAJOR.MINOR` version series, such as `1.19`.
...
The [desktop application](https://github.com/OpenwaterHealth/openlifu-desktop-application)'s releases determine the support boundaries for SlicerOpenLIFU,
as described in the [desktop application support schedule](https://github.com/OpenwaterHealth/openlifu-desktop-application/blob/main/SUPPORT.md):
...
The graphic and table describe release lines. Always use the latest patch in a supported line.
...
These follow the [desktop application's temporary exceptions](https://github.com/OpenwaterHealth/openlifu-desktop-application/blob/main/SUPPORT.md#temporary-exceptions):

- The latest "legacy IO" release line is fully supported for now.
- SlicerOpenLIFU `v1.18` is Unsupported, rather than receiving Critical Support, because desktop application `v1.11` is Unsupported.
