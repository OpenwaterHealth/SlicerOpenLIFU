"""OpenLIFU Home page — landing dashboard for the OpenLIFU host module.
...
class OpenLIFUHomeWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def on_sign_in_clicked(self, checked: bool = False) -> None:
        from OpenLIFUApp.pages_legacy.data_page import _ModuleWidgetPopupDialog
    ...
    def on_new_session_clicked(self, checked: bool) -> None:
        ...
        from OpenLIFUApp.pages_legacy.data_page import CreateNewSessionDialog, LoadSubjectDialog
...
# The former ``OpenLIFUHomeTest`` (with ``runTest`` / ``_ensure_dvc_gdrive_support`` /
# ``get_test_database`` / ``_OpenLIFU_FullTest1`` orchestrating every page's Test
# class) was folded into ``OpenLIFUTest`` in ``OpenLIFU.py`` by Round 5c-3
# Commit D when the ``OpenLIFUHome`` shim module was deleted.
