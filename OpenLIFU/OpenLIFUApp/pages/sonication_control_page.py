"""Sonication control page for the OpenLIFU host module.

Extracted from the former standalone ``OpenLIFUSonicationControl`` scripted
module during the de-moduling migration (Round 2 of DEMODULING.md). The
Slicer module shell ``OpenLIFUSonicationControl/OpenLIFUSonicationControl.py``
still exists as a thin adapter so navigation and
``slicer.util.getModuleLogic(...)`` calls (used by module_layout,
OpenLIFUData, and OpenLIFUHome) keep working until Round 5 folds page
navigation into the host.
"""
...
class OpenLIFUSonicationControlWidget(ScriptedLoadableModuleWidget, VTKObservationMixin, GuidedWorkflowMixin):
    ...
    def onViewRunsClicked(self, checked: bool = False) -> None:
        ...
        from OpenLIFUData import RunManagerDialog
